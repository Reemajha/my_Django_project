...
def get_chart(chart_type, data, results_by, **kwargs):
    plt.switch_backend('AGG') #important line
    ...
    if chart_type == '#1':
        ...
        print('pie chart')
        plt.pie(data=d, x='total_price', labels=d[key].values) #getting the labels directly on the based of values from the Key
    elif chart_type == '#3':
        print('line chart')
        plt.plot(d[key], d['total_price'], color='green', marker='o', linestyle ='dashed')
    else:
        print("Oops! Failed to identify the chart type")
    plt.tight_layout() #adjust the size of the response accoridng to the fig size
    chart = get_graph()
    return chart